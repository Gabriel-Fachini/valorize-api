...
**Plan & Billing**:
- `GET /:id/plan` - Get company current plan information
...
12. `getPlan()` - Get company current active plan
13. `updatePlan()` - Change plan type or pricing
14. `getBillingInfo()` - Current MRR and next billing date
15. `getMetrics()` - Users, compliments, engagement, redemptions, values
16. `addContact()` - Link existing user to company
17. `updateContact()` - Update role or primary status
18. `deleteContact()` - Unlink user from company
19. `addAllowedDomain()` - Add SSO domain
20. `deleteAllowedDomain()` - Remove SSO domain
